<?php
	...
	$uniqu_code = Qcformbuilder_Forms_Field_Util::generate_file_field_unique_id($field,$form);
	...
	if( empty( $field['config']['multi_upload_text'] ) ){
		$field['config']['multi_upload_text'] = __( 'Add File', 'qcformbuilder-forms' );
		if( !empty( $field['config']['multi_upload'] ) ){
	...
	//Fix allowed types
    // @see https://github.com/QcformbuilderWP/Qcformbuilder-Forms/issues/2471
    if ( ! empty( $field['config']['allowed'] )) {
	...
	$field['config']['notices'] = array(
		'file_exceeds_size_limit' => esc_html__( 'File exceeds the maximum upload size for this site.', 'qcformbuilder-forms' ),
		'zero_byte_file' => esc_html__( 'This file is empty. Please try another.', 'qcformbuilder-forms' ),
		'invalid_filetype' => esc_html__( 'This file type is not allowed. Please try another.', 'qcformbuilder-forms' ),
	);
	...
	<?php echo $field_label; ?>
		<?php echo $field_before; ?>
			<div
					...
					class="wfb-multi-uploader-list"
			></div>

			<button
					id="<?php echo esc_attr($field_id); ?>_trigger"
					type="button"
					class="btn btn-block wfb-uploader-trigger"

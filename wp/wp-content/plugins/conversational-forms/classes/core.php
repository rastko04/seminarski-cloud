...
class Qcformbuilder_Forms
{
	...
	public static function activate_qcformbuilder_forms($force = false)
	{
		...
		$columns = $wpdb->get_results("SHOW COLUMNS FROM `" . $wpdb->prefix . "wfb_form_entry_values`", ARRAY_A);
		$fields = array();
		...
		if (!in_array('field_id', $fields)) {
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "wfb_form_entry_values` ADD `field_id` varchar(20) NOT NULL AFTER `entry_id`;");
			$wpdb->query("CREATE INDEX `field_id` ON `" . $wpdb->prefix . "wfb_form_entry_values` (`field_id`); ");
			// update all entries
			$forms = $wpdb->get_results("SELECT `id`,`form_id` FROM `" . $wpdb->prefix . "wfb_form_entries`", ARRAY_A);
			$known = array();
			if (!empty($forms)) {
		...
		// add status
		$columns = $wpdb->get_results("SHOW COLUMNS FROM `" . $wpdb->prefix . "wfb_form_entries`", ARRAY_A);
		$fields = array();
		...
		if (!in_array('status', $fields) && $version < '1.2.0') {
			$wpdb->query("ALTER TABLE `" . $wpdb->prefix . "wfb_form_entries` ADD `status` varchar(20) NOT NULL DEFAULT 'active' AFTER `datestamp`;");
			$wpdb->query("CREATE INDEX `status` ON `" . $wpdb->prefix . "wfb_form_entries` (`status`); ");
		}
	...
	static public function get_entry_meta($entry_id, $form, $type = null)
	{
		...
		$entry_meta_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM `" . $wpdb->prefix . "wfb_form_entry_meta` WHERE `entry_id` = %d",
			$entry_id), ARRAY_A);
	...
	static public function process_submission()
	{
		...
		if (empty($transdata['edit']) && !empty($form['db_support'])) {
			...
			$wpdb->insert($wpdb->prefix . 'wfb_form_entries', $new_entry);
			...
			if (isset($form_processors[$processor['type']])) {
				...
				if (!empty($transdata['error'])) {
					...
					if (!empty($entryid) && !empty($new_entry) && $new_entry['status'] == 'pending') {
						// kill it with fire
						$wpdb->delete($wpdb->prefix . 'wfb_form_entries', array('id' => $entryid));
					}
					// set error transient
		...
		if (!empty($form['form_draft'])) {
			if (!isset($_GET['wfb_preview']) || $_GET['wfb_preview'] != $form['ID']) {
				if (isset($_POST['action']) && $_POST['action'] == 'wfb_get_form_preview') {
					echo '<p style="color: #cf0000;">' . __('Form is currently not active.', 'qcformbuilder-forms') . '</p>';
				} else {
					return;
				}

			} else {
				echo '<div class="qcformbuilder-grid"><p class="alert alert-error alert-danger">' . __('Form is currently not active.',
						'qcformbuilder-forms') . '</p></div>';
			}
		}

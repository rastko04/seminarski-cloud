 ...
 * 2014 - David Cramer
 */
if( !class_exists( 'Qcformbuilder_Form_Grid' )){
	
	class Qcformbuilder_Form_Grid {
		...
		public function renderLayout($grid = false) {
			...
			foreach($grid as $row=>$cols){
				...
				/**
				 * Alter row ID attribute in Qcformbuilder Grid
				 *
				 * @since 1.4.9
				 *
				 ...
				 */
				$rowID = apply_filters( 'qcformbuilder_forms_grid_row_id', $rowID, $row, $this->config );
				...
				/**
				 * Alter row class attribute in Qcformbuilder Grid
				 *
				 ...
				 * @param array $config Grid config. Contains form ID in form_id key.
				 */
				$rowClass = apply_filters( 'qcformbuilder_forms_grid_row_class', $rowClass, $row, $this->config );

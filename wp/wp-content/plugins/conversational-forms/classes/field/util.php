 ...
 */
class Qcformbuilder_Forms_Field_Util {
	...
	public static function is_file_field( $field, array $form = null ){
		...
		return in_array( self::get_type( $field, $form ), Qcformbuilder_Forms_Files::types() );
	}
	...
	public static function prepare_field_classes( $field, $form ){
		...
		if ( empty( self::$field_classes[ $form[ 'ID' ] ][ $current_form_count ][ $field[ 'ID' ] ] ) ) {
			...
			$field_classes = apply_filters( 'qcformbuilder_forms_render_field_classes', $field_classes, $field, $form );
			$field_classes = apply_filters( 'qcformbuilder_forms_render_field_classes_type-' . $field[ 'type' ], $field_classes, $field, $form );
			$field_classes = apply_filters( 'qcformbuilder_forms_render_field_classes_slug-' . $field[ 'slug' ], $field_classes, $field, $form );
	...
	public static function get_math_functions( array $form ){
		 ...
		 */
		return apply_filters( 'qcformbuilder_forms_field_util_math_functions', $math_functions, $form );
	}
	...
	public static function apply_field_filters( $field, $form ){
		 ...
		 *
		 * Filter name is dynamic, based on field type. For example "qcformbuilder_forms_render_get_field_type-hidden" or "qcformbuilder_forms_render_get_field_type-radio"
		 *
		 * @since unknown
		 *
		 * @param array $field The field config.
		 ...
		 *
		 * Filter name is dynamic, based on field type. For example "qcformbuilder_forms_render_get_field_slug-salsa" or "qcformbuilder_forms_render_get_field_slug-chips"
		 *
		 * @since unknown
		 *
		 * @param array $field The field config.
	...
	public static function credit_card_types( $field, $form ){
		 ...
		 * @param array $form Form config
		 */
		return apply_filters( 'qcformbuilder_forms_credit_card_types', $types, $field, $form );
	}
	...
	public static function get_default_calc_value( $field, $form ){
		...
		if( is_array( $option_values ) && isset( $field['config']['option'] ) && count( $field['config']['option'] ) > 0 ){
			...
			foreach( $field['config']['option'] as $op ){
                if (Qcformbuilder_Forms_Field_Util::is_checked_option($op[ 'value'], $option_values)) {
	...
	public static function check_conditional( $field, array $form ){
		...
		if ( ! empty( $field[ 'conditions' ][ 'type' ] ) ) {
			...
			return Qcformbuilder_Forms::check_condition( $conditional, $form );
		}
	...
	public static function get_option_calculation_value( $option, array $field, array  $form ){
		 ...
		 * @param array $form Form configuration
		 */
		return apply_filters( 'qcformbuilder_forms_get_option_calculation_value', $calc_val, $option, $field, $form );
	}
	...
	public static function find_select_field_value( $field, $field_value ){
		...
		if ( ! empty( $field[ 'config' ][ 'option' ] ) ) {
			$option_values = Qcformbuilder_Forms_Field_Util::find_option_values( $field );

			if( isset( $field['config'] ) && ! empty( $field[ 'config' ][ 'default_option' ] ) ) {
				$field_value = Qcformbuilder_Forms::do_magic_tags(  $field[ 'config' ][ 'default_option' ] );

...
class handler extends AbstractHandler
{
	...
	protected function high_enough_level($level)
	{
		 ...
		 *
		 */
		$highest = apply_filters('qcformbuilder_forms_pro_minimum_level',
	...
	protected function prepare(array $record)
	{
		...
		if ( method_exists('Qcformbuilder_Forms_DB_Tables',
	...
	protected function is_recent_repeat($message)
	{
		if ( empty($this->tracking_key) ) {
			$this->tracking_key = qcformbuilder_forms_pro_log_tracker_key(CF_PRO_VER);
		}
		...
		$this->recents[] = $hash;
		\Qcformbuilder_Forms_Transient::set_transient($this->tracking_key, $this->recents);
		return false;
	...
	protected function get_recent()
	{
		if ( empty($this->recents) ) {
			$this->recents = \Qcformbuilder_Forms_Transient::get_transient($this->tracking_key);

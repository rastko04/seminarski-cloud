...
if ( ! class_exists( 'WP_Dark_Mode_Enqueue' ) ) {
	...
	class WP_Dark_Mode_Enqueue {
		...
		public function __construct() {
			...
			add_action( 'script_loader_tag', [ $this, 'modify_script_tag' ], 10, 2 );
		}
		/**
		 ...
		 * @version 1.0.0
		 */
		public function modify_script_tag( $tag, $handle ) {

			$allowed_handlers = [ 'wp-dark-mode-admin', 'wp-dark-mode-js', 'wp-dark-mode-js-defer', 'wp-dark-mode-core' ];
			if ( in_array( $handle, $allowed_handlers ) ) {
				return str_replace( ' src', ' nowprocket data-no-minify="1" src', $tag );
			}
